...
class DSRDataset(BaseDataset):  # Nikon Camera Images
	def __init__(self, opt, split='train', dataset_name='DSR'):
		super(DSRDataset, self).__init__(opt, split, dataset_name)
		if self.root == '':
			rootlist = ['']
		...
		self.camera = {'canon':['IMG','Canon','0510'], 'sony':['sony'],
					   'nikon':['DSC'], 'oly':['P11'], 'pan':['pan']}

		self.x_scale = 'x' + str(opt.scale)
		if split == 'train':
			self.train_root = os.path.join(self.root, 'Nikon', 'train_')
			self.lr_dirs, self.hr_dirs, self.names = self._get_image_dir(self.train_root, self.camera['nikon'])
			self.len_data = 1000 * self.batch_size  # len(self.names)
			self._getitem = self._getitem_train

		elif split == 'val':
			self.val_root = os.path.join(self.root, 'Nikon', 'test_')
			self.lr_dirs, self.hr_dirs, self.names = self._get_image_dir(self.val_root, self.camera['nikon']) 
			self._getitem = self._getitem_val
			self.len_data = len(self.names)

		elif split == 'test':
			self.test_root = os.path.join(self.root, 'Nikon', 'test_')
			self.lr_dirs, self.hr_dirs, self.names = self._get_image_dir(self.test_root, self.camera['nikon']) 
	...
	def _getitem_train(self, idx):
		...
		hr_bili_noise, degradation_list = degrade_kernel(hr_trans)
		noise = hr_bili_noise
	...
	def _crop_center(self, lr, hr, fw=0.25, fh=0.25, p=0):
		ih, iw = lr.shape[-2:]
		if p != 0:
			fw = p / iw
			fh = p / ih
